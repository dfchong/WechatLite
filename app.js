...
App({
  ...
  apiUrl: 'https://a.recruit.donesz.cn/addons/recruit/',
  Domain_Public:'https://a.recruit.donesz.cn/',
  Isdebug:true,
  si: 0,
  //小程序启动
  onLaunch: function () {
    if (this.Isdebug){
      this.QrPngUrl = 'https://www.fa.com/addons/recruit/';
    }
    var that = this;
    ...
    wx.getLocation({
      ...
      success: function (lb) {
        ...
        wx.request({ // ②百度地图API，将微信获得的经纬度传给百度，获得城市等信息
          url: 'https://api.map.baidu.com/geocoder/v2/?ak='+that.globalData.config.baidukey+'&location=' + lb.latitude + ',' + lb.longitude + '&output=json&coordtype=wgs84ll',
          data: {},
          header: {
  ...
  Makeloc: function (L_data) {
    var that = this;
    if(that.globalData.city.length === 0){
      that.error('后台无开放区域');
      return
    }
    if (L_data!='no'){
